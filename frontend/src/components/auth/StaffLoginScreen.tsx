...
export default function StaffLoginScreen({ onDemoMode }: StaffLoginScreenProps) {
  const { login, loginStatus, identity } = useInternetIdentity();

  const isLoggingIn = loginStatus === 'logging-in';
  const isAuthenticated = !!identity;

  const demoRoles: { role: AppRole; label: string; icon: React.ReactNode; color: string }[] = [
    { role: 'phlebotomist', label: 'Phlebotomist', icon: <FlaskConical className="h-4 w-4" />, color: 'bg-blue-50 border-blue-200 text-blue-700' },
    { role: 'labAdmin', label: 'Lab Admin', icon: <BarChart3 className="h-4 w-4" />, color: 'bg-purple-50 border-purple-200 text-purple-700' },
    { role: 'superAdmin', label: 'Super Admin', icon: <Shield className="h-4 w-4" />, color: 'bg-red-50 border-red-200 text-red-700' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-primary/10 flex flex-col items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-sm space-y-6">
        ...
        <div className="bg-muted/50 rounded-xl p-3 space-y-1.5">
          ...
          {[
            { icon: <FlaskConical className="h-3 w-3" />, label: 'Phlebotomist — Sample collection & attendance' },
            { icon: <BarChart3 className="h-3 w-3" />, label: 'Lab Admin — Bookings, reports & billing' },
            { icon: <Shield className="h-3 w-3" />, label: 'Super Admin — Full system access' },
          ].map((item, i) => (
        ...
        {onDemoMode && (
          <>
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-semibold text-muted-foreground">OR TRY DEMO MODE</span>
              <div className="flex-1 h-px bg-border" />
            </div>
