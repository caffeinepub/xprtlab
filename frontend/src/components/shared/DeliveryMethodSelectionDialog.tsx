...
import { DeliveryMethod } from '../../types/models';
...
const DELIVERY_OPTIONS: Array<{ value: DeliveryMethod; label: string; description: string }> = [
  { value: 'WHATSAPP', label: 'WhatsApp', description: 'Send report via WhatsApp message' },
  { value: 'PHYSICAL', label: 'Physical Delivery', description: 'Hand-deliver the printed report' },
  { value: 'EMAIL', label: 'Email', description: 'Send report via email' },
  { value: 'HOSPITAL_PICKUP', label: 'Hospital Pickup', description: 'Patient collects from hospital' },
];
        ...
        >
          {DELIVERY_OPTIONS.map((opt) => (
            <div key={opt.value} className="flex items-start gap-3">
              ...
              <Label htmlFor={`delivery-${opt.value}`} className="cursor-pointer">
