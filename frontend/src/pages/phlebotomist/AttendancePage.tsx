...
import { useGetTodaySummary, useStartShift, useEndShift } from '../../hooks/useQueries';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function AttendancePage() {
  const { data: summary, isLoading: summaryLoading, error: summaryError } = useGetTodaySummary();
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-md bg-card rounded-2xl shadow-card p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">Shift Ended</h2>
          <p className="text-muted-foreground mb-6">Your shift has been closed. All records are locked for today.</p>
          <div className="bg-muted rounded-xl p-4 space-y-2 text-sm text-left">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-success">₹{cashCollected.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">UPI Collected</span>
              <span className="font-semibold text-blue-600">₹{upiCollected.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Pending Amount</span>
              <span className="font-semibold text-warning">₹{pendingAmount.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="bg-card border-b border-border px-4 py-4 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">Today's Attendance</h1>
      ...
      <div className="p-4 space-y-4 max-w-lg mx-auto">
        ...
        <Alert>
          ...
          <AlertDescription>
            Shift is active. Once you end the shift, all records for today will be locked and no new samples can be added.
          </AlertDescription>
        </Alert>
        ...
        {summaryError && (
          <Alert variant="destructive">
            ...
            <AlertDescription>Failed to load today's summary. Please refresh.</AlertDescription>
          </Alert>
        )}
            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription asChild>
                <div className="space-y-3">
                  <p>Once you end your shift, all records for today will be locked. You will not be able to add, edit, or delete samples for today.</p>
                  <div className="bg-muted rounded-lg p-3 space-y-1.5 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Cash Collected</span>
                      <span className="font-semibold">₹{cashCollected.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">UPI Collected</span>
                      <span className="font-semibold">₹{upiCollected.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Pending Amount</span>
                      <span className="font-semibold text-warning">₹{pendingAmount.toFixed(2)}</span>
