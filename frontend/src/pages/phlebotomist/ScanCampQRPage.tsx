...
import { QrCode, Camera, CheckCircle, AlertCircle, RotateCcw, StopCircle } from 'lucide-react';
  ...
  return (
    <div className="px-4 py-5 space-y-4 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Scan QR code to log camp check-in</p>
      </div>
      ...
      <div>
        ...
        {qrResults.length === 0 ? (
          <MedicalCard className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground">No QR codes scanned yet</p>
          </MedicalCard>
        ) : (
          <div className="space-y-2">
            {qrResults.map((result) => (
              <MedicalCard key={result.timestamp} className="border-green-200 bg-green-50">
