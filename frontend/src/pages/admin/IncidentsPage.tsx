...
export default function IncidentsPage({ onNavigate }: IncidentsPageProps) {
  ...
  const getSeverityKey = (severity: any): string => {
    ...
    if (typeof severity === 'object' && severity !== null) return Object.keys(severity)[0];
    return 'low';
  };
        ...
        {(['all', 'low', 'medium', 'high'] as SeverityFilter[]).map(s => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-semibold transition-colors ${
              severityFilter === s ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-muted/80'
            }`}
          >
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${severityColors[sevKey]}`}>
                    {sevKey.toUpperCase()}
                  </span>
                  <span className="text-xs text-muted-foreground">{formatTime(Number(incident.timestamp))}</span>
                </div>
                ...
                {incident.photo && (
                  <div className="mt-2">
                    <img
                      src={typeof incident.photo === 'string' ? incident.photo : incident.photo?.getDirectURL?.()}
                      alt="Incident photo"
