...
type BookingStatusFilter = 'all' | 'pending' | 'confirmed' | 'completed' | 'canceled';
    ...
    <div className="p-4 space-y-4 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm p-4 space-y-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {(['all', 'pending', 'confirmed', 'completed', 'canceled'] as BookingStatusFilter[]).map(s => (
            <button
              key={s}
            ...
            return (
              <div key={booking.id} className="bg-white rounded-2xl border border-border shadow-sm p-4 space-y-2">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColors[statusKey] || 'bg-gray-100 text-gray-600'}`}>
                    {statusKey}
                  </span>
                </div>
                ...
                <p className="text-xs text-muted-foreground">Slot: {booking.slot}</p>
                <p className="text-xs text-muted-foreground">{formatTime(Number(booking.timestamp))}</p>
                  ...
                  {['confirmed', 'completed', 'canceled'].map(s => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-xl text-xs font-semibold border transition-colors disabled:opacity-40 ${
                        statusKey === s ? 'bg-primary/10 border-primary/20 text-primary' : 'border-border hover:bg-muted/50'
                      }`}
                    >
                      {s.charAt(0).toUpperCase() + s.slice(1)}
                    </button>
