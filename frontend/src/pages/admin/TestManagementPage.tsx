...
import { Search, Plus, Pencil, Ban, TestTube, ChevronLeft, ChevronRight, Upload, Loader2 } from 'lucide-react';
...
import { useGetAllTests, useBulkAddTests, useSetTestStatus } from '../../hooks/useQueries';
import { TestInput, TestOutput, TestError } from '../../backend';
...
function parseCSV(content: string): { rows: ParsedCSVRow[]; parseErrors: number } {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!testName || !testCode || !sampleType || !mrpRaw || isNaN(mrp) || mrp < 0) {
      parseErrors++;
      continue;
    }
...
export default function TestManagementPage({ role }: TestManagementPageProps) {
  ...
  const handleToggleStatus = async (test: TestOutput, newIsActive: boolean) => {
    ...
    try {
      const result = await setTestStatusMutation.mutateAsync({ testId, isActive: newIsActive });
      if (result.__kind__ === 'err') {
        toast.error('Failed to update test status.');
      } else {
        toast.success(`Test "${test.name}" marked as ${newIsActive ? 'Active' : 'Inactive'}.`);
      }
    } catch (err: any) {
      toast.error(`Failed to update test status: ${err?.message ?? String(err)}`);
    } finally {
      setTogglingIds((prev) => {
  ...
  const handleCSVFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        toast.success(`${added.length} test${added.length !== 1 ? 's' : ''} added successfully.`);
        if (totalSkipped > 0) {
          toast.warning(`${totalSkipped} row${totalSkipped !== 1 ? 's' : ''} skipped (duplicates or invalid data).`);
        }
      } catch (err: any) {
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold text-foreground">Test Management</h1>
        </div>
        {isSuperAdmin && (
          <div className="flex items-center gap-2">
            <Button
            ...
            {isLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell><Skeleton className="h-4 w-16" /></TableCell>
                  <TableCell><Skeleton className="h-4 w-16" /></TableCell>
                  {isSuperAdmin && <TableCell><Skeleton className="h-4 w-10 mx-auto" /></TableCell>}
                  {isSuperAdmin && <TableCell><Skeleton className="h-4 w-20 ml-auto" /></TableCell>}
                </TableRow>
              ))
            ) : isError ? (
              <TableRow>
                <TableCell colSpan={isSuperAdmin ? 7 : 5} className="text-center text-destructive py-8">
              ...
              <TableRow>
                <TableCell colSpan={isSuperAdmin ? 7 : 5} className="text-center text-muted-foreground py-8">
                  {searchQuery ? 'No tests match your search.' : 'No tests found. Add your first test.'}
                </TableCell>
              </TableRow>
            ) : (
              paginatedTests.map((test) => {
                ...
                return (
                  <TableRow key={test.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="font-medium">{formatMRP(test.price)}</TableCell>
                    <TableCell>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>
            Showing {(currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, filteredTests.length)} of {filteredTests.length}
          </span>
