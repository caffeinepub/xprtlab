      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gradient-to-r from-primary to-primary/80">
            <th className="text-left px-4 py-3 text-white font-bold text-xs">
              Module
            </th>
            {roles.map((r) => (
              <th
                key={r.key}
                className="px-3 py-3 text-white font-bold text-xs text-center"
              >
                {r.label}
              </th>
            ))}
