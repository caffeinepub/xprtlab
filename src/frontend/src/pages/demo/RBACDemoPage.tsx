...
const roleDescriptions: Record<
  AppRole,
  { title: string; responsibilities: string[] }
> = {
  patient: {
    title: "Patient",
    responsibilities: [
...
export default function RBACDemoPage() {
  ...
  return (
    <div className="p-4 space-y-6 max-w-2xl mx-auto">
      ...
      {/* Role Overview */}
      <div
        className={`rounded-2xl border p-4 space-y-2 ${roleColors[selectedRole]}`}
      >
        <h3 className="font-bold text-sm">{desc.title} Role</h3>
        <ul className="space-y-1">
          {desc.responsibilities.map((r) => (
