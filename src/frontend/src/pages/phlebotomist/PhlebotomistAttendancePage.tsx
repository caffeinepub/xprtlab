      ...
      {status === "loading" && (
        <div className="flex flex-col items-center gap-3 py-8">
          ...
          <p className="text-xs text-gray-400 text-center">
            Please stay still and ensure GPS is enabled.
          </p>
        </div>
      )}
          ...
          {location.isMock && (
            <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100">
              ...
              <p className="text-sm text-amber-700">
                Mock/spoofed location detected. This will be flagged in the
                attendance record.
              </p>
            </div>
          )}
...
function ActiveShiftPanel({
  ...
  const [gpsStatus, setGpsStatus] = useState<"idle" | "loading" | "done">(
    "idle",
  );
...
}: PhlebotomistAttendancePageProps) {
  const [startStep, setStartStep] = useState<StartStep>("gps");
  const [gpsLocation, setGpsLocation] = useState<GPSLocation | null>(null);
  const [selfieUrl, setSelfieUrl] = useState<string>("");
  const [activeShift, setActiveShift] = useState<ShiftRecord | null>(null);
  const [completedShift, setCompletedShift] = useState<ShiftRecord | null>(
    null,
  );
  ...
  const handleEndShift = (endLocation: GPSLocation) => {
    ...
    setTimeout(() => {
      ...
      const totalMinutes = Math.round(
        (endTime - activeShift.startTime) / 60000,
      );
