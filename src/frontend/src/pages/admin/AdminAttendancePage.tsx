...
import {
  useGetAllActiveShifts,
  useGetAttendanceByPhlebotomist,
...
export default function AdminAttendancePage({
  ...
  const { data: activeShifts, isLoading: activeLoading } =
    useGetAllActiveShifts();
  const { data: allAttendances = [], isLoading: historyLoading } =
    useGetAttendanceByPhlebotomist();

  // Normalize active shifts to array
  const activeShiftsArray: any[] = activeShifts
    ? Array.isArray(activeShifts)
      ? activeShifts
      : [activeShifts]
    : [];
  ...
  useEffect(() => {
    const update = () => {
      ...
      for (const shift of activeShiftsArray as any[]) {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-600">
                      Since {formatTime(Number(shift.checkInTime))}
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-2">
            {filteredHistory.map((att: any, i: number) => (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  In: {formatTime(Number(att.checkInTime))}
                </p>
                {att.checkOutTime && (
                  <p className="text-xs text-muted-foreground">
                    Out: {formatTime(Number(att.checkOutTime))}
                  </p>
                )}
                {att.totalWorkingMinutes && (
                  <p className="text-xs font-semibold text-primary">
                    {att.totalWorkingMinutes} min worked
                  </p>
                )}
              </div>
            ))}
