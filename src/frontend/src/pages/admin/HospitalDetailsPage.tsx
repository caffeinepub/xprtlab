...
export default function HospitalDetailsPage({
  ...
  const { data: hospital, isLoading: hospitalLoading } =
  ...
  const [allPhlebotomists, setAllPhlebotomists] = useState<
    Array<{ principal: string; profile: UserProfile }>
  >([]);
  ...
  const filteredPhlebotomists = useMemo(() => {
    ...
    return allPhlebotomists.filter((p) => {
      ...
      return (
        ...
        (name.includes(term) ||
          area.includes(term) ||
          p.principal.toLowerCase().includes(term))
      );
    });
            ...
            <div className="border border-border rounded-lg p-3 space-y-3 bg-muted/20">
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                  {filteredPhlebotomists.map((p) => {
                    ...
                    const sameArea =
                      hospital.area &&
                      p.profile.area &&
                      hospital.area.toLowerCase() ===
                        (p.profile.area ?? "").toLowerCase();
                    return (
                      <button
                        type="button"
                        key={p.principal}
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={
                    selectedToAssign.length === 0 || assignMutation.isPending
                  }
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{hospital.name}</strong>? The assignment history will be
              preserved.
            </AlertDialogDescription>
          </AlertDialogHeader>
...
function AssignedPhlebotomistRow({
  ...
  React.useEffect(() => {
    if (!actor) return;
    actor
      .getUserProfile(Principal.fromText(principalStr))
      .then((p) => {
        if (p) setProfile(p);
      })
      .catch(() => {});
  ...
  const sameArea =
    hospitalArea &&
    profile?.area &&
    hospitalArea.toLowerCase() === (profile.area ?? "").toLowerCase();
