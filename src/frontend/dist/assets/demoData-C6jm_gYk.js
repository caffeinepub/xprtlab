import { d as createLucideIcon } from "./ProfileSetupModal-C_51Oj8J.js";
/**
...
const DEMO_HOSPITAL_ID_3 = "demo-hospital-vijaya";
...
function buildDefaultRegisteredUsers() {
  return [
    {
      id: "test-phlebo-001",
      name: "Test Phlebo",
...
function addDemoSample(sample) {
  const all = safeRead(KEYS.SAMPLES) ?? [];
  const withId = sample.sampleId ? sample : { ...sample, sampleId: generateSampleId() };
  all.push(withId);
...
function getRegisteredUser(mobile) {
  const all = safeRead(KEYS.REGISTERED_USERS) ?? [];
...
function getSession() {
  try {
    ...
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (Date.now() - parsed.loginAt > 8 * 60 * 60 * 1e3) {
...
function generateSampleId() {
  ...
  try {
    ...
    return `XRPT-${dateStr}-${Math.floor(Math.random() * 9e3 + 1e3)}`;
  }
}
...
export {
  DEMO_PHLEBO_ID as D,
  Package as P,
  getDemoHomeCollections as a,
  getSession as b,
  clearSession as c,
  getDemoSamples as d,
  getDemoHomeCollections$1 as e,
  getDemoSamples$1 as f,
  getRegisteredUser as g,
  getDemoHospitalsByPhlebotomist as h,
  initializeDemoStorage as i,
  addDemoSample as j,
  getDemoHospitals as k,
  getDemoStatusHistory as l,
  updateDemoSampleDelivery as m,
  updateDemoSampleStatus as n,
  addDemoDeliveryTracking as o,
  addDemoTestMaster as p,
  updateDemoTestMaster as q,
  getDemoSettlements as r,
  saveSession as s,
  saveDemoSettlement as t,
